...
Hello Game 是一个基于 Vue 3 + Vite 构建的寓教于乐游戏平台，集益智、休闲、早教与互动体验于一体，让每个人都能在游戏中发现乐趣。
...
| 游戏        | 说明                             |
|-----------|--------------------------------|
| 💰 实时工资   | 实时工资计算器，按秒展示赚到的钱，金币掉落特效        |
| 🍽️ 今天吃什么 | 美食随机抽取，选择餐次口味忌口，华丽老虎机动画帮你决定吃什么 |
...
- **实时工资计算**：双模式（智能模式按系统时间实时计薪 / 自由模式从零计时），按秒更新金额，每满 ¥10 金币掉落特效，工作状态实时提示
- **今天吃什么**：选择餐次（早/午/晚饭）、口味偏好和忌口，老虎机式华丽滚动动画随机抽取美食，粒子特效 + 彩纸飘落结果展示，支持自定义添加食物并本地持久化
...
│   │   └── Header.vue
│   ├── composables/
│   │   ├── useFoodPicker.js        # 今天吃什么本地存储
│   │   └── useTimer.js             # 计时器组合式函数
│   ├── data/
│   │   ├── foodPicker.js           # 今天吃什么预置食物数据
│   │   ├── fortune.js              # 算命数据
│   │   ├── games.js                # 游戏列表与分类配置
...
- **趣味休闲**：实时工资、今天吃什么、牛马测试、算命运势，轻松有趣
- **火影互动**：摄像头手势识别结印，沉浸式忍者体验
